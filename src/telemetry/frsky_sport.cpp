  ...
  switch (id) {
    ...
    case GPS_ALT_AP_ID:
      if (frskyData.hub.gpsFix) {
        ...
        if (!frskyData.hub.baroAltitudeOffset) {
          int altitude = frskyData.hub.gpsAltitude_bp / 100;
  ...
  switch (prim)
  {
    case DATA_FRAME:
      ...
      if (appId == SWR_ID) {
        ...
        frskyData.hub.gpsAltitude_bp = SPORT_DATA_S32(packet);
        ...
        if (frskyData.hub.gpsLatitude_bp && frskyData.hub.gpsLongitude_ap) {
          frskyData.hub.gpsFix = 1;
        }
